...
def greedy_decode(model, src_tensor, max_len,device):
    ...
    encode_out = model.encode(src_tensor.to(device), src_mask)
        ...
        with tqdm(dataloader, unit="batch", desc="eval") as pbar:
            for i, (src_batch, tgt_batch) in enumerate(pbar):
                ...
                for src in src_batch:
                    ...
                    translated_tensor = greedy_decode(model, src.unsqueeze(0), max_len, device)
...
def train(model, train_loader, optimizer, criterion, num_epochs=10,eval_loader=None,src_pad_id=0,tgt_pad_id=0):
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(train_loader, unit="batch", desc=f"Epoch {epoch+1}/{num_epochs}") as pbar:
            for i, (src, tgt) in enumerate(pbar):
                ...
                if (i + 1) % 10000 == 0:  # 每10000个batch打印一次损失
                    ...
                    logging.info(f"Epoch [{epoch+1}/{num_epochs}], Step [{i+1}/{len(train_loader)}], Loss: {total_loss / (i+1):.4f}")
        ...
        torch.save(model.state_dict(), f"./checkpoint/transformer_epoch_{epoch+1}.pth")
        # 每个epoch结束后，进行验证
        if eval_loader:
            ...
            logging.info(f"BLEU: {bleu_score * 100:.2f}%")
...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(train_dataset, batch_size=8,num_workers=0,drop_last=False, shuffle=True,collate_fn=collate_fn)
    ...
    val_loader = DataLoader(val_dataset, batch_size=1,num_workers=0, shuffle=False,collate_fn=collate_fn)
